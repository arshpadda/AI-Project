...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
        #print grad_tensors
        # Follow the formula given in the website.
        for counter in range(0, len(param_vars)):
            old_value = param_vars[counter]
            new_value = old_value - self.learning_rate * grad_tensors[counter]
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for counter in range(0, len(param_vars)):
            new_weight = self.momentum * vel_vars[counter] - self.learning_rate * grad_tensors[counter]
            updates.append([vel_vars[counter],new_weight])
            new_value = param_vars[counter] + new_weight
            updates.append([param_vars[counter],new_value])
        #util.raiseNotDefined()
        return updates
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        #Look at the example in tensorflow_util. Helps a lot!
        training_mini_batch = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        validation_mini_batch = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            #util.raiseNotDefined()
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            training_mini_batch_input, training_mini_batch_target = training_mini_batch.next()
            train_loss = session.run(loss_tensor, feed_dict={model.input_ph : training_mini_batch_input, target_ph : training_mini_batch_target})
            session.run(update_ops, feed_dict={model.input_ph : training_mini_batch_input, target_ph : training_mini_batch_target})
            ...
            validation_mini_batch_input, validation_mini_batch_target = validation_mini_batch.next()
            val_loss = session.run(loss_tensor, feed_dict={model.input_ph : validation_mini_batch_input, target_ph : validation_mini_batch_target})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        training_mini_batch = MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        validation_mini_batch = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            #util.raiseNotDefined()
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
