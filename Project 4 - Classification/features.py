...
def enhancedFeatureExtractor(datum):
    ...
    features_copy = np.array(features)
    #The final enhanced_feature will depend on the number of white spaces.
    enhanced_features = np.zeros(3, dtype=int)
    number_of_white_spaces = 0

    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if features_copy[y + x * DIGIT_DATUM_HEIGHT] == 0:
                number_of_white_spaces = number_of_white_spaces + 1
                depth_first_search(x,y,features_copy)

    if number_of_white_spaces == 1:
        enhanced_features[0] = 1
    elif number_of_white_spaces == 2:
        enhanced_features[1] = 1
    else:
        enhanced_features[2] = 1

    #util.raiseNotDefined()
    return np.append(features, enhanced_features)
    #return features

def depth_first_search(x,y,features_copy):

    if x + 1 < DIGIT_DATUM_WIDTH:
        if features_copy[DIGIT_DATUM_HEIGHT * (x + 1) + y] == 0:
            features_copy[DIGIT_DATUM_HEIGHT * (x + 1) + y] = 1
            depth_first_search(x + 1, y, features_copy)
    ...
    if y + 1 < DIGIT_DATUM_HEIGHT:
        if features_copy[DIGIT_DATUM_HEIGHT * x + (y + 1)] == 0:
            features_copy[DIGIT_DATUM_HEIGHT * x + (y + 1)] = 1
            depth_first_search(x, y + 1, features_copy)
